...
1. **查概念** → 第一个调用必须是 `lookup_concept(user_request)`，把用户问题传进去。**例外：如果用户消息中明确列出了文件 ID（如 `[xxx] filename`），说明用户已选定文件进行分析（分析模式），此时直接跳到步骤 2，用消息中列出的文件 ID 调用 `read_file(file_id)`，不要调用 lookup_concept。**
...
- **每次回答必须包含 KB 参考信息**：在回答末尾用「📚 NetApp KB 相关文章」小节列出 search_kb 返回的文章标题和链接
"""
