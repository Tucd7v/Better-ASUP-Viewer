...
## 绘制拓扑图 / 架构图
如果用户要求绘制拓扑图、网络图或架构图，请用 Mermaid 格式：
- 使用 \`\`\`mermaid 代码块
- 拓扑图用 `graph TD` 或 `flowchart TB`
- 节点用方括号 [名称] 表示，连线用 `-->` 或 `---`
- 示例：
\`\`\`mermaid
graph TD
    ClusterA[ClusterA<br/>192.168.1.1] --> Switch1[Switch1]
    Switch1 --> ClusterB[ClusterB<br/>192.168.1.2]
    ClusterA --> Node1[Node1]
    Switch1 --> Node2[Node2]
    ClusterB --> Node3[Node3]
\`\`\`
不要用 HTML、不要用 ASCII 画图，只用 Mermaid。
"""
