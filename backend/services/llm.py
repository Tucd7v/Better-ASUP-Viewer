...
2. **读文件** → concept 返回的 `matched_files` 里已经包含真实的文件 ID，直接用 `read_file(file_id)` 打开。不要自己从 catalog 里找文件——matched_files 就是正确答案。如果 `found: false`，用 `find_files(pattern)` 按文件名搜索 catalog 获取 file_id，再调 read_file。
