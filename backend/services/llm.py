...
**数量/统计类问题（几个、有哪些、多少）直接读文件，禁止 search_logs！**——catalog 和 concept 已经告诉你文件在哪，读完文件从数据里数出来就行。

### 第 3 步：输出结论（读完后立刻回答，禁止继续搜索）
read_file 返回结果后，**立即**基于已有数据输出分析结论。不要追加 search_logs、不要再去读"可能相关"的文件。
用户问什么答什么，不要发散到用户没问的领域。

### search_logs 使用规则（极少使用）
**仅在以下情况下调用 search_logs：**
- 用户明确要求搜索某个关键词/错误码（如"搜所有文件里的 timeout"）
- lookup_concept 和 catalog 中都找不到任何相关文件名
- ❌ 禁止：读了一堆文件后"再搜搜看有没有遗漏"——读完就输出！
- ❌ 禁止：用 search_logs 去"找哪些文件相关"——文件名已经在 catalog 里了
...
TOOLS = [
    {
        ...
        "function": {
            "name": "search_logs",
            "description": "⚠️ 搜索文件内容（不是文件名）。仅用于用户明确要求搜索关键词/错误码的场景。禁止用于发现文件——文件在哪由 catalog 和 lookup_concept 告诉你。数量统计类问题（几个、有哪些）绝对不要调用此工具。",
            "parameters": {
                "type": "object",
                "properties": {
